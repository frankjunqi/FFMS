...
<html>
  ...
  <body>
  	...
  	<center><h1>备忘录</h1></center>
  	<div align="right"><a href="other/addremind.jsp">添加备忘录</a></div>
			...
			<table class="datagrid" summary="Listado de cobros en MercadoPago">
				...
				<thead>
					<tr>
						...
						<th scope="col">编号 : </th>
						<th scope="col">备忘内容</th>
						<th scope="col" class="selected">备忘时间<span class="asc">(asc)</span></th>
				...
				<c:forEach items="${requestScope.rl}" var="RemindInFo">
					<tr>
						<td scope="row"><input type="checkbox" name="mycheck" value="${RemindInFo.rem_id}"></td>
						<td> <c:out value="${RemindInFo.rem_content}"/> </td>
						<td><c:out value="${RemindInFo.rem_remindtime}"/> </td>
						<td><c:out value="${RemindInFo.rem_createtime}"/> </td>
						<td><a href="DeleteRemindServlet?rem_id=${RemindInFo.rem_id}" >删除</a>|
	  	  <a href="other/updateremind.jsp?rem_id=${RemindInFo.rem_id }&rtime=${RemindInFo.rem_remindtime}">修改</a></td>
